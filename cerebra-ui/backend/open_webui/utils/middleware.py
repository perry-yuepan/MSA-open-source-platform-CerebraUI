...
async def chat_image_generation_handler(
    ...
    #####insert
    # Get configuration for Smart Mode decision
    chat_id = form_data.get("chat_id")
    if not chat_id:
        # Fallback: get from metadata (most reliable)
        chat_id = extra_params.get("__metadata__", {}).get("chat_id")
    engine = request.app.state.config.IMAGE_GENERATION_ENGINE
    enable_smart = bool(request.app.state.config.ENABLE_FAL_SMART_MODE)
    ##########

    try:
        # Decision: Smart API or Traditional?
        if engine == "fal" and enable_smart and chat_id:
            # ===== SMART MODE PATH =====
            log.info(f"[SmartGen] Using Smart Mode for chat {chat_id}")
            
            # TODO: Smart Mode implementation (Part 3)
            # Update status with analysis message
            await __event_emitter__({
                "type": "status",
                "data": {
                    "description": "🧠 Analyzing prompt and generating image...",
                    "done": False
                }
            })
            
            # Import Smart API components
            from open_webui.routers.images import (
                smart_image_generation,
                SmartGenerateImageForm
            )
            
            # Set OpenAI key for analyzer if configured
            if request.app.state.config.FAL_OPENAI_API_KEY:
                import os
                os.environ["OPENAI_API_KEY"] = request.app.state.config.FAL_OPENAI_API_KEY
            
            # Call Smart API
            smart_result = await smart_image_generation(
                request=request,
                form_data=SmartGenerateImageForm(
                    chat_id=chat_id,
                    prompt=prompt,
                    image_size=request.app.state.config.FAL_DEFAULT_IMAGE_SIZE,
            ...
            # Convert Smart API response to middleware format
            images = [{"url": smart_result["url"]}]
            
            # Generate enhanced status message with mode info
            mode_emoji = "🎨" if smart_result["mode"] == "text2img" else "✨"
            mode_desc = "text-to-image" if smart_result["mode"] == "text2img" else "image-to-image"
            
            await __event_emitter__({
                "type": "status",
                "data": {
                    "description": f"{mode_emoji} Generated using {mode_desc}",
                    "done": True
                }
            })
            ...
            await __event_emitter__({
                "type": "files",
                "data": {
                    "files": [{"type": "image", "url": smart_result["url"]}]
                }
            })
            
            system_message_content = "<context>User is shown the generated image, tell the user that the image has been generated</context>"
            
            log.info(f"[SmartGen] Success: mode={smart_result['mode']} session={smart_result['session_id']}")
            
        else:
            # ===== TRADITIONAL PATH =====
            if engine == "fal" and not enable_smart:
                log.info("[FalFlux] Using basic mode (Smart disabled)")
            elif engine == "fal" and not chat_id:
                log.warning("[FalFlux] No chat_id, using basic mode")
        ...
        # Smart Mode specific error (preserves HTTP status codes)
        log.exception(e)
        ...
        await __event_emitter__(
            {
                "type": "status",
                "data": {
                    "description": f"❌ Image generation failed: {error_detail}",
                    "done": True
                }
            }
        )
        ...
        # General error handling (keeps original behavior)
        log.exception(e)
        await __event_emitter__(
            {
                "type": "status",
