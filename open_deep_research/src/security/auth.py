...
# # for every request. This will determine whether the request is allowed or not
# @auth.authenticate
# async def get_current_user(authorization: Optional[str] = None) -> Auth.types.MinimalUserDict:
#     """
#     Local-dev (no Supabase configured): allow all requests without Authorization header.
#     Prod (Supabase configured): require 'Authorization: Bearer <jwt>' and validate with Supabase.
#     """

#     # ---- Local dev: skip auth entirely if Supabase isn't configured ----
#     if not supabase:
#         return {"identity": "local-dev-user"}

#     # ---- Prod: require Authorization header ----
#     if not authorization:
#         raise Auth.exceptions.HTTPException(
#             status_code=401, detail="Authorization header missing"
#         )
